...
} = require('./map/ZTEMP3_mappings.js');
const util = require('./util');

class ThermostatFourModeDevice extends ZwaveDevice {
  async onNodeInit() {
    this.registerCapability('thermostat_mode', 'THERMOSTAT_MODE', {
      ...
      setParserV3: thermostatMode => {
        ...
        if (this.getCapabilityValue('thermostat_mode') !== thermostatMode) {
          ...
          this.homey.app.triggerThermostatModeChangedTo.trigger(this, null, thermostatModeObj).catch(this.error);

          // 4. Update onoff state when the thermostat mode is off
          if (thermostatMode === 'Off') {
            this.setCapabilityValue('thermostat_state_13570', false).catch(this.error);
          }

        }
        // 5. Return setParser object and update thermostat_mode capability
        return {
          Level: {
      ...
      reportParserV3: report => {
        ...
        if (report.hasOwnProperty('Level') && report.Level.hasOwnProperty('Mode')) {
          ...
          this.setCapabilityValue('onoff', thermostatMode === 'Heat').catch(this.error);

          // 5. Return reportParser object and update thermostat_mode capability
          return thermostatMode;
        }
        return null;
      },
      multiChannelNodeId: this.capabilityMultiChannelNodeIdObj['thermostat_state_13570'],
    });
    ...
    this.registerCapability('target_temperature', 'THERMOSTAT_SETPOINT', {
      ...
      multiChannelNodeId: this.capabilityMultiChannelNodeIdObj['thermostat_mode'],
    });
    ...
    this.registerCapability('thermostat_state_13570', 'THERMOSTAT_OPERATING_STATE', {
      ...
      reportParser: report => {
        if (report && report.hasOwnProperty('Level') && report.Level.hasOwnProperty('Operating State')) {
          ...
          if (thermostatState !== this.getCapabilityValue('thermostat_state_13570')) {
  ...
  async onCapabilityOnoff(value, opts) {
    await this.executeCapabilitySetCommand('thermostat_mode', 'THERMOSTAT_MODE', value ? 'Heat' : 'Off')
  ...
  async onModeAutocomplete(query, args) {
    ...
    for (const modeID in Mode2Number) {
      resultArray.push({
        id: modeID,
        name: util.getModeName(this.homey, modeID),
...
module.exports = ThermostatFourModeDevice;
