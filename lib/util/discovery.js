 ...
 */
async function discoverDevices({ driverName, isModelMatch, log }) {
    ...
    const scanPromises = range.map(async (i) => {
        const ip = baseIp + i;
        try {
            const isOnline = await util.checkTcpConnection(ip, 80, 5000);
            if (isOnline) {
                const data = await getDeviceData(ip, isModelMatch, log);
                if (data.matched) {
                    return {
                        Ip: ip,
                        Mac: data.mac,
 ...
 */
async function getDeviceData(ip, isModelMatch, log) {
