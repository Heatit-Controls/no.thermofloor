...
async function checkTcpConnection(hostname, port = 80, timeout = 2000) {
...
/**
 * Normalize a thermostat mode to a string id.
 * @param {*} mode - Mode as reported by the device
 * @returns {string} Mode id, empty string when the mode is missing
 */
function getModeId(mode) {
    if (typeof mode === 'string') return mode;
    if (typeof mode === 'number') return String(mode);
    if (mode !== null && typeof mode === 'object') {
        if (typeof mode.id === 'string') return mode.id;
        if (typeof mode.mode === 'string') return mode.mode;
        return '';
    }
    return '';
}
...
/**
 * Translate a thermostat mode into a human readable name.
 * Falls back to the mode id when no (string) translation is available, so the
 * result is always usable as a Flow token.
 * @param {Object} homey - Homey instance providing the __() translation method
 * @param {*} mode - Mode as reported by the device
 * @returns {string} Translated mode name
 */
function getModeName(homey, mode) {
    const modeId = getModeId(mode);
    if (!modeId) return '';

    const translation = homey.__(`mode.${modeId}`);
    return typeof translation === 'string' && translation.length > 0 ? translation : modeId;
}

/**
 * Build the Flow token object for the thermostat mode trigger cards.
 * @param {Object} homey - Homey instance providing the __() translation method
 * @param {*} mode - Mode as reported by the device
 * @returns {{mode: string, mode_name: string}} Tokens, both guaranteed strings
 */
function getModeTokens(homey, mode) {
    const modeId = getModeId(mode);
    return {
        mode: modeId,
        mode_name: getModeName(homey, modeId),
    };
}
