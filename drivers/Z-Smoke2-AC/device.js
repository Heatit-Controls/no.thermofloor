...
module.exports = class ZSmokeACDevice extends ZwaveDevice {
  async onNodeInit() {
    ...
    this.registerCapability('alarm_maintenance', 'NOTIFICATION', {
      get: 'NOTIFICATION_GET',
      getParser: () => ({
        'V1 Alarm Type': 0,
      ...
      reportParser: report => {
        if (report && (report['Notification Type'] === 'Smoke')) {
          if (report['Event (Parsed)'] === 'Force Maintenance') {
            this.log('Dust force maintenance active');
            return true;
          }

          if (report['Event (Parsed)'] === 'Event inactive') {
            this.log('Dust force maintenance cleared');
            return false;
          }
        }
        return null;
