...
class ZMThermostat2Device extends ZwaveDevice {
	constructor(...args) {
		super(...args);
		...
		this.registerCapability('thermostat_mode', 'THERMOSTAT_MODE', {
			...
			setParser: value => {
				if (value === 'Powerregulator') {
					...
					if (value !== 'off' && settings?.operating_mode === '4') {
						this.handleExitPowerRegulatorMode(value);
					} else if (value !== 'off') {
	...
	async handleExitPowerRegulatorMode(newMode) {
		// Restore normal target temperature options
		await this.setCapabilityOptions('target_temperature', this._originalTempOptions);

		// When leaving PWER, set the Operating mode (MODE) parameter accordingly
		const operatingModeIndex = this.getParameterIndex('operating_mode') || 27;
		const map = {
			'off': 0,
			'heat': 1,
			'cool': 2,
		...
		const desired = map[newMode] !== undefined ? map[newMode] : 1;
		await this.configurationSet({ index: operatingModeIndex, size: 1, signed: false }, desired).catch(this.error);
		await this.setSettings({ operating_mode: String(desired) }).catch(this.error);
	}
	...
	async handleEnterPowerRegulatorMode() {
		try {
			...
			// Set Operating mode (MODE) parameter to PWER (4)
			const operatingModeIndex = this.getParameterIndex('operating_mode') || 27;
			await this.configurationSet({ index: operatingModeIndex, size: 1, signed: false }, 4);
			await this.setSettings({ operating_mode: '4' });
		} catch (error) {
	...
	async initializeDeviceState() {
		try {
			...
			const operatingMode = parseInt(settings.operating_mode, 10);
	...
	async onSettings({ oldSettings, newSettings, changedKeys }) {
		...
		if (changedKeys.includes('operating_mode')) {
			const opMode = parseInt(newSettings.operating_mode, 10);
			if (opMode === 4) {
				await this.handleEnterPowerRegulatorMode();
				await this.setCapabilityValue('thermostat_mode', 'Powerregulator');
				const powerRegValue = this.getPowerRegulatorValue();
				await this.setCapabilityValue('target_temperature', powerRegValue * 10).catch(this.error);
			} else if (this.getCapabilityValue('thermostat_mode') === 'Powerregulator') {
	...
	handleThermostatModeReport(mode) {
		...
		const settings = this.getSettings();
		if (settings?.operating_mode === '4' && capabilityMode !== 'off') {
	...
	async handleConfigurationReport(report) {
		try {
			...
			} else if (paramNum === this.getParameterIndex('operating_mode')) {
				await this.setSettings({ operating_mode: String(parsedValue) });
				if (parsedValue === 4) {
					await this.handleEnterPowerRegulatorMode();
					await this.setCapabilityValue('thermostat_mode', 'Powerregulator');
					const powerRegValue = this.getPowerRegulatorValue();
...
module.exports = ZMThermostat2Device;
